...
template <typename T> void *operator new(size_t s, T *v) { return v; }
...
template <typename T> struct Simple_alloc {
  ...
  // memory allocation
  T *allocate(int n) { return reinterpret_cast<T *>(new char[n * sizeof(T)]); }
  void deallocate(T *p, int n) { delete[] reinterpret_cast<char *>(p); }
...
//                               Vector
//*********************************************************************************
template <class T, class A = Simple_alloc<T>> class Vector {
...
public:
  ...
  Vector(const int s) : sz(0) { reserve(s); }
